...
function applyLanguageToUI(){
  ...
  const customLabel = document.querySelector('label[for="customUrl"]');
  if (customLabel) customLabel.innerText = t('ui.customUrl');
  document.querySelector('label[for="volume"]').innerText  = t('ui.ambientVol');

  // Chip label
  const chipSpan = document.querySelector('.chip span');
  if (chipSpan) chipSpan.innerText = t('ui.voiceCues');
...
function playAmbient(){
  ...
  if (choice === 'none') {
    stopAmbient();
    setStatus(t('ui.statusOff'));
    return;
  }
  let src = (choice === 'custom') ? (customUrl.value||'').trim() : sounds[choice];
  if (!src){
    setStatus(t('ui.statusNoSrc'));
    return;
  }
  if (!unlocked){
    setStatus(t('ui.statusEnable'));
    return;
  }

  if (audio && audio.dataset.src === src){
    fadeTo(audio, targetVolume, 600);
    setStatus(t('ui.statusOn'));
    return;
  }
...
// show/hide + enable voice volume controls
...
function speak(phaseKey){
  ...
  u.rate = 0.95;
  u.pitch = 1.05;
  u.volume = voiceVolume;   // use adjustable volume
...
function getPattern(){
  switch (patternEl.value){
    case 'box':
      return { phases:[['Inhale',4,'expand'],['Hold',4,''],['Exhale',4,'shrink'],['Hold',4,'']], cycle:16 };
    case '478':
      return { phases:[['Inhale',4,'expand'],['Hold',7,''],['Exhale',8,'shrink']], cycle:19 };
    case 'coherent':
      return { phases:[['Inhale',5,'expand'],['Exhale',5,'shrink']], cycle:10 };
    case 'triangle':
    default:
      return { phases:[['Inhale',4,'expand'],['Hold',4,''],['Exhale',4,'shrink']], cycle:12 };
  }
}
...
function start(){
  ...
  startBtn.textContent = t('ui.pause');
...
function pauseResume(){
  ...
  setPrompt(
    paused ? t('ui.paused') : I18N[LANG].speak[currentPattern.phases[phaseIndex][0]],
